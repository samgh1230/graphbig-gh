...
typedef struct my_worklist
{
    ...
    void init(unsigned marker)
    {
	cudaErrCheck(cudaMallocMark((void**)&item_array, WORKLIST_SIZE*sizeof(uint64_t), marker) );
	cudaErrCheck(cudaMalloc((void**)&end, sizeof(uint32_t)) );
	clear();
    }
    void clear(void)
        ...
        uint64_t root)
{
    ...
    // malloc of gpu side
    cudaErrCheck( cudaMallocMark((void**)&device_vpl, vertex_cnt*sizeof(uint32_t),3) );
	...
	cudaErrCheck( cudaUpdateWlBand(wl_size));
	cudaErrCheck( cudaStartPrefetch());
        kernel<<<num_block_chunked, num_thread_per_block>>>(
                device_vpl, d_graph, *in_worklist, *out_worklist, curr);
        cudaErrCheck( cudaEndPrefetch());
